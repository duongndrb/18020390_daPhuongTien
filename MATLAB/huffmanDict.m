function output = huffmanDict(inputimg)
intensity = imhist(inputimg);
index = 1;
...
function output = setTable(root, table)
    if root.Data >= 0
...
function output = sort_last_node(nodes)
    l = length(nodes);
    i = l;
    while i ~= 1,
...
function output = sort_all_node(nodes)
    l = length(nodes);
    for i=1 : l-1,
        min = i;
        for j=i+1 : l,
